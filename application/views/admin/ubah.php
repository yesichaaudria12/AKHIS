							...
							<div class="form-group row my-2">
								...
								<div class="col-lg-9">
									...
									<select class="form-control input-rounded" id="sel1" name="<?= $c->name; ?>">
										<option value="<?= $obat[$c->name] ? $obat[$c->name] : set_value($c->name); ?>"><?= $obat[$c->name] ? $obat[$c->name] :"Silahkan Pilih ". ucwords($c->name); ?></option>
									...
									<div class="input-group">
										...
										<?php endif; ?>
										<input type="<?= isset($type) ? $type[$i] : "text"; ?>" class="form-control input-rounded" name="<?= $c->name; ?>" id="<?= $c->name; ?>" placeholder="Masukan <?= str_replace("_", " ", $c->name); ?>" <?= isset($add_atribut) ? $add_atribut[$i] : ""; ?> value="<?= $obat[$c->name] ? $obat[$c->name] : set_value($c->name); ?>">
									</div>
									<?= form_error($c->name, '<small class="pl-3 text-danger">', '</small>'); ?>
									<?php endif; ?>
								</div>
