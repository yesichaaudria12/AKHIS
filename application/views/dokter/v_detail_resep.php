	...
	<div class="page-content">
						...
						<?php if ($detail_resep) : ?>
							<?php foreach ($detail_resep as $dr) : ?>
								<div class="row align-items-center border mx-3">
									<div class="col" data-bs-toggle="modal" data-bs-target="#detail<?= $dr['id_obat'] ?>" style="cursor: pointer;">
										<img src="<?= base_url('assets'); ?>/img/obat/<?= $dr['gambar']; ?>" alt="gambar obat" width="100">
									</div>
									<div class="col text-start" data-bs-toggle="modal" data-bs-target="#detail<?= $dr['id_obat'] ?>" style="cursor: pointer;">
										<p><?= $dr['nama_obat']; ?></p>
									</div>
									<div class="col mb-1">
										<button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#ubah<?= $dr['id']; ?>">Ubah</button>
										<button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#hapus<?= $dr['id']; ?>">hapus</button>
									</div>
								</div>
								<!-- ubah obat -->
								<div class="modal fade" id="ubah<?= $dr['id']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
													...
													<div class="card-body p-5">
														...
														<form action="<?= base_url('dokter/ubah/resep_obat/' . $id_pasien . '/' . $dr['id']); ?>" method="POST" class="row row-cols-1 g-3 text-start">
															<div class="col">
																...
																<input type="text" name="nama_obat" class="form-control" id="nama_obat" value="<?= $dr['nama_obat']; ?>" readonly>
															</div>
															<div class="col">
																<label for="Qty" class="form-label">Jumlah obat</label>
																<input type="text" name="Qty" class="form-control" id="Qty" value="<?= $dr['Qty'] ?>">
																<?= form_error('Qty', '<small class="text-danger">', '</small>'); ?>
															</div>
															<div class="col">
																<div class="form-floating">
																	<textarea class="form-control" placeholder="Aturan Minum" name="aturan_minum" id="aturan_minum" style="height: 100px"><?= $dr['aturan_minum'] ?></textarea>
								...
								<div class="modal fade" id="hapus<?= $dr['id']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
		...
		<div class="row row-cols-1 row-cols-sm-2 row-cols-lg-3 row-cols-xl-4 row-cols-xxl-5 product-grid">
			<?php foreach ($obat as $obat) : ?>
				<div class="col">
					<div class="card">
						...
						<div class="card-footer text-center">
							<a href="javascrpt:;" class="btn btn-sm btn-primary px-5" data-bs-toggle="modal" data-bs-target="#pilih<?= $obat['id_obat']; ?>">pilih</a>
						</div>
					</div>
				</div>

				<!-- detail Obat Modal -->
				<div class="modal fade" id="detail<?= $obat['id_obat']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
				...
				<!-- modal pilih -->
				<div class="modal fade" id="pilih<?= $obat['id_obat']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
					<div class="modal-dialog modal-lg">
						<div class="modal-content">
							<div class="modal-body">
								<div class="card border-top border-0 border-4 border-primary">
									<div class="card-body p-5">
										...
										<form action="<?= current_url(); ?>" method="POST" class="row row-cols-1 g-3">
											<div class="col">
												<label for="nama_obat" class="form-label">Nama Obat</label>
												<input type="text" name="nama_obat" class="form-control" id="nama_obat" value="<?= $obat['nama_obat']; ?>" readonly>
											</div>
											<div class="col">
												<label for="Qty" class="form-label">Jumlah obat</label>
												<input type="text" name="Qty" class="form-control" id="Qty" value="<?= set_value('Qty'); ?>">
												<?= form_error('Qty', '<small class="text-danger">', '</small>'); ?>
											</div>
											<div class="col">
												<div class="form-floating">
													<textarea class="form-control" placeholder="Aturan Minum" name="aturan_minum" id="aturan_minum" style="height: 100px"><?= set_value('aturan_minum'); ?></textarea>
													<label for="aturan_minum">Aturan Minum</label>
													<?= form_error('aturan_minum', '<small class="text-danger">', '</small>'); ?>
												</div>
											</div>
